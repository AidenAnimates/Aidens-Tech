...
import net.mcreator.aidenstechnology.procedures.PumpProcedureProcedure;
...
public record GasPumpGUIButtonMessage(int buttonID, int x, int y, int z) implements CustomPacketPayload {
	...
	}, (RegistryFriendlyByteBuf buffer) -> new GasPumpGUIButtonMessage(buffer.readInt(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
	@Override
	public Type<GasPumpGUIButtonMessage> type() {
		return TYPE;
	}

	public static void handleData(final GasPumpGUIButtonMessage message, final IPayloadContext context) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = GasPumpGUIMenu.guistate;
		// security measure to prevent arbitrary chunk generation
		if (!world.hasChunkAt(new BlockPos(x, y, z)))
			return;
		if (buttonID == 0) {

			PumpProcedureProcedure.execute(entity);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		AidensTechnologyMod.addNetworkMessage(GasPumpGUIButtonMessage.TYPE, GasPumpGUIButtonMessage.STREAM_CODEC, GasPumpGUIButtonMessage::handleData);
	}
}
